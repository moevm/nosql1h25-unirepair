...
const generateData = (brigadesCount, operatorsCount, adminsCount, callFormsCount, reportsCount, inventoryCount) => {
  ...
  if (usersEmpty) {
    for (let brigadeNumber = 1; brigadeNumber <= brigadesCount; brigadeNumber++) {
      users.push({
        role: 'Brigadier',
  ...
  if (callFormsEmpty) {
    for (let i = 1; i <= callFormsCount; i++) {
      callForms.push({
        ...
        bottomLeft: { latitude: 59.9 + Math.random() * 0.1, longitude: 30.3 + Math.random() * 0.1 },
        topRight: { latitude: 59.9 + Math.random() * 0.1 + 0.005, longitude: 30.3 + Math.random() * 0.1 + 0.005 },
        fireType: ['заводской пожар', 'жилой дом', 'лесной пожар'][Math.floor(Math.random() * 3)],
        fireRank: `${Math.floor(Math.random() * 4) + 1}`,
        victimsCount: Math.floor(Math.random() * 10),
        assignedTo: Array.from({ length: Math.floor(Math.random() * 3) + 1 }, () => Math.floor(Math.random() * brigadesCount) + 1),
  ...
  if (reportsEmpty) {
    ...
    const causes = ["неосторожное обращение с огнём", "поджог", "неисправность электропроводки", "неизвестно"];
    ...
    for (let i = 1; i <= reportsCount; i++) {
      reports.push({
        ...
        additionalNotes: Math.random() < 0.5 ? "Пожар был локализован быстро." : "Требуется дополнительное расследование.",
        modifiedAt: new Date().toISOString()
      });
    }
  }
  ...
  if (usersEmpty) fs.writeFileSync(usersFile, JSON.stringify(users, null, 2), 'utf8');
  if (callFormsEmpty) fs.writeFileSync(callFormsFile, JSON.stringify(callForms, null, 2), 'utf8');
  if (reportsEmpty) fs.writeFileSync(reportsFile, JSON.stringify(reports, null, 2), 'utf8');
  if (inventoryEmpty) fs.writeFileSync(inventoryFile, JSON.stringify(inventory, null, 2), 'utf8');
